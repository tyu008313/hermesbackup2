...
5. Proxy-node routing: Store only public upstream metadata (protocol, host, port, country/city, enabled state). Resolve credentials from a secret/env source at runtime; never return, log, or persist them in the public registry. For multiple clients, generate one Xray outbound per selected node and route by the client identity (UUID/email), not by a single global proxy selection; otherwise every client inherits the last selected node.
6. Health semantics: distinguish TCP reachability from protocol authentication and end-to-end traffic. Never report a proxy country or traffic success from metadata alone; label unverified geography and require a real probe or external verification before claiming it.
7. Release integrity: after any source or auth change, run the full suite, syntax/build validation, and rebuild the ZIP. Treat any archive created before the last change as stale. Flatten Railway output when required, and exclude tests, caches, binaries, data, and build artifacts from the deployable archive.
8. Verify: Use a Node harness that stubs `fetch` and KV. Run the project’s real test suite before packaging; a registry count or a passing mocked unit test is not evidence that every advertised feature works.
9. Enforce Telegram/Cloudflare limits: every `callback_data` ≤ 64 bytes.
